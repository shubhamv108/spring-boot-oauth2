...
define del-local-app
    @docker stop spring-boot-oauth
    @docker rm spring-boot-oauth
endef
...
rm-images: clean
	...
	docker image rm shubham01/spring-boot-oauth
	docker image rm spring-boot-oauth

docker-build:
	docker build -t shubham01/spring-boot-oauth:latest .
...
run: build
	docker run -p 8080:8080 shubham01/spring-boot-oauth:latest --network="host"
